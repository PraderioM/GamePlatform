...
async def end_game(request: web.Request) -> web.Response:
    async def get_game_from_database(db: asyncpg.Connection) -> Game:
        game_data = await get_game_data(game_id=request.rel_url.query['game_id'], db=db)
        return Game.from_database(game_data)

    return await general_end_game(pool=request.app['db'], game_id=request.rel_url.query['game_id'],
